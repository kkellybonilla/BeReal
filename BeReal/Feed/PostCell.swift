    ...
    func configure(with post: Post) {
        ...
        if let currentUser = User.current,
           ...
           let diffHours = Calendar.current.dateComponents([.hour], from: postCreatedDate, to: lastPostedDate).hour {
            ...
            // Hide the image view if the given post was created within 24 hours of the current user's last post. (before or after)
            postImageView.isHidden = abs(diffHours) > 24
        } else {
